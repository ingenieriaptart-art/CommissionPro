...
const SECTION_TORQUE: MockInspectionSection = {
  id: "sec-torque",
  code: "CALIBRACION_TORQUE",
  name: "Calibración y Verificación de Torque / Par",
  is_universal: false,
  fields: [
    { key: "tipo_sensor",        label: "Tipo de sensor",                  type: "select",   required: true,  options: ["Extensómetro en árbol (strain gauge)", "Magnetoestrictivo no invasivo", "Óptico (encoder de torsión)", "Capacitivo rotativo", "Torquímetro en línea (flange)", "Transductor de par estático"] },
    { key: "variable_medida",    label: "Variable medida",                 type: "select",   required: true,  options: ["Par / torque (N·m)", "Par específico (N·m/kg)", "Potencia mecánica (kW)", "Potencia mecánica (hp)"] },
    ...
    { key: "unidad_ing",         label: "Unidad de ingeniería",            type: "select",   required: true,  options: ["N·m", "kN·m", "lbf·ft", "lbf·in", "kgf·m"] },
    { key: "senal_salida",       label: "Señal de salida",                 type: "select",   required: true,  options: ["4-20 mA HART", "4-20 mA", "mV/V (puente Wheatstone)", "Modbus RTU", "CAN bus", "Frecuencia"] },
    { key: "tipo_montaje",       label: "Tipo de montaje",                 type: "select",   required: true,  options: ["En línea — árbol rotativo", "Brida rígida", "Externo no invasivo (sin contacto)", "Estático (banco de pruebas)"] },
    { key: "velocidad_medicion", label: "Velocidad durante medición (RPM)",type: "numero",   required: false, validations: { unit: "RPM" } },
    { key: "sensibilidad_mv_v",  label: "Sensibilidad del transductor (mV/V)", type: "numero", required: false, validations: { unit: "mV/V" } },
    { key: "umbral_alarma",      label: "Setpoint ALARMA sobretorque",     type: "numero",   required: true },
    { key: "umbral_trip",        label: "Setpoint PARO / TRIP sobretorque",type: "numero",   required: true },
    { key: "cal_0_aplicado",     label: "Par aplicado  0 % (LRV)",        type: "numero",   required: true },
    { key: "cal_0_leido",        label: "Señal leída  0 %",               type: "numero",   required: true },
    { key: "cal_25_aplicado",    label: "Par aplicado 25 %",               type: "numero",   required: true },
    { key: "cal_25_leido",       label: "Señal leída 25 %",               type: "numero",   required: true },
    { key: "cal_50_aplicado",    label: "Par aplicado 50 %",               type: "numero",   required: true },
    { key: "cal_50_leido",       label: "Señal leída 50 %",               type: "numero",   required: true },
    { key: "cal_75_aplicado",    label: "Par aplicado 75 %",               type: "numero",   required: true },
    { key: "cal_75_leido",       label: "Señal leída 75 %",               type: "numero",   required: true },
    { key: "cal_100_aplicado",   label: "Par aplicado 100 % (URV)",       type: "numero",   required: true },
    { key: "cal_100_leido",      label: "Señal leída 100 %",              type: "numero",   required: true },
    { key: "histeresis_medida",  label: "Histéresis medida (%)",           type: "numero",   required: false, validations: { unit: "%" } },
    { key: "func_trip_ok",       label: "Función PARO por sobretorque verificada", type: "checkbox", required: true, options: ["OK", "FALLA", "N/A"] },
    { key: "ajuste_cero",        label: "Ajuste de cero realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "ajuste_span",        label: "Ajuste de span realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "foto_evidencia",     label: "Foto torquímetro / evidencia",    type: "imagen",   required: false },
    { key: "obs_torque",         label: "Observaciones",                   type: "textarea", required: false },
    { key: "resultado_torque",   label: "Resultado",                       type: "select",   required: true,  options: ["APROBADO", "RECHAZADO"] },
  ],
};

const SECTION_PESO_FUERZA: MockInspectionSection = {
  id: "sec-peso-fuerza",
  code: "CALIBRACION_PESO_FUERZA",
  name: "Calibración y Verificación de Peso / Fuerza",
  is_universal: false,
  fields: [
    { key: "tipo_sensor",        label: "Tipo de sensor / celda",          type: "select",   required: true,  options: ["Celda de carga — compresión (strain gauge)", "Celda de carga — tracción (strain gauge)", "Celda de carga — cizalladura", "Sensor piezoeléctrico", "Magnetoestrictivo", "Balanza de plataforma", "Pesómetro de cinta (belt scale)"] },
    { key: "variable_medida",    label: "Variable medida",                 type: "select",   required: true,  options: ["Peso (kg)", "Peso (t)", "Fuerza (N)", "Fuerza (kN)", "Presión de carga (bar)"] },
    { key: "rango_min",          label: "Rango mínimo LRV",                type: "numero",   required: true },
    { key: "rango_max",          label: "Rango máximo URV / capacidad",    type: "numero",   required: true },
    { key: "unidad_ing",         label: "Unidad de ingeniería",            type: "select",   required: true,  options: ["kg", "t", "N", "kN", "MN", "lbf", "kgf"] },
    { key: "senal_salida",       label: "Señal de salida",                 type: "select",   required: true,  options: ["4-20 mA HART", "4-20 mA", "mV/V (Wheatstone)", "Modbus RTU", "Profibus DP", "Interruptor (switch)"] },
    { key: "num_celdas",         label: "Número de celdas en paralelo",    type: "numero",   required: false, validations: { min: 1 } },
    { key: "sensibilidad_mv_v",  label: "Sensibilidad combinada (mV/V)",   type: "numero",   required: false, validations: { unit: "mV/V" } },
    { key: "clase_exactitud",    label: "Clase de exactitud (OIML / NTEP)",type: "select",   required: false, options: ["Clase A (analítica)", "Clase B (precisión)", "Clase C (industrial)", "Clase D (gruesa)"] },
    { key: "tara",               label: "Tara / carga muerta (kg)",        type: "numero",   required: false, validations: { unit: "kg" } },
    { key: "umbral_alarma",      label: "Setpoint ALARMA sobrecarga",      type: "numero",   required: false },
    { key: "umbral_trip",        label: "Setpoint PARO / TRIP sobrecarga", type: "numero",   required: false },
    { key: "cal_0_aplicado",     label: "Carga aplicada  0 % (cero/tara)",type: "numero",   required: true },
    { key: "cal_0_leido",        label: "Lectura instrumento  0 %",        type: "numero",   required: true },
    { key: "cal_25_aplicado",    label: "Carga aplicada 25 %",             type: "numero",   required: true },
    { key: "cal_25_leido",       label: "Lectura instrumento 25 %",        type: "numero",   required: true },
    { key: "cal_50_aplicado",    label: "Carga aplicada 50 %",             type: "numero",   required: true },
    { key: "cal_50_leido",       label: "Lectura instrumento 50 %",        type: "numero",   required: true },
    { key: "cal_75_aplicado",    label: "Carga aplicada 75 %",             type: "numero",   required: true },
    { key: "cal_75_leido",       label: "Lectura instrumento 75 %",        type: "numero",   required: true },
    { key: "cal_100_aplicado",   label: "Carga aplicada 100 % (URV)",      type: "numero",   required: true },
    { key: "cal_100_leido",      label: "Lectura instrumento 100 %",       type: "numero",   required: true },
    { key: "corner_test_ok",     label: "Corner test (plataforma) realizado", type: "checkbox", required: false, options: ["OK", "FALLA", "N/A"] },
    { key: "deriva_cero",        label: "Deriva en cero tras 5 min (%)",   type: "numero",   required: false, validations: { unit: "%" } },
    { key: "error_max_pct",      label: "Error máximo admisible (%)",      type: "numero",   required: false, validations: { unit: "%", min: 0 } },
    { key: "ajuste_cero",        label: "Ajuste de cero / tara realizado", type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "ajuste_span",        label: "Ajuste de span realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "foto_pesas_patron",  label: "Foto pesas patrón / evidencia",   type: "imagen",   required: false },
    { key: "obs_peso",           label: "Observaciones",                   type: "textarea", required: false },
    { key: "resultado_peso",     label: "Resultado",                       type: "select",   required: true,  options: ["APROBADO", "RECHAZADO"] },
  ],
};

const SECTION_VELOCIDAD: MockInspectionSection = {
  id: "sec-velocidad",
  code: "CALIBRACION_VELOCIDAD",
  name: "Calibración y Verificación de Velocidad / Velocimetría",
  is_universal: false,
  fields: [
    { key: "tipo_sensor",        label: "Tipo de sensor",                  type: "select",   required: true,  options: ["Magnético de reluctancia variable (VR)", "Efecto Hall", "Encoder de pulsos óptico", "Encoder de pulsos magnético", "Taco-generador analógico", "Sensor óptico / laser", "Radar Doppler"] },
    { key: "variable_medida",    label: "Variable medida",                 type: "select",   required: true,  options: ["Velocidad rotacional (RPM)", "Frecuencia (Hz)", "Velocidad lineal (m/s)", "Velocidad angular (rad/s)"] },
    { key: "rango_min",          label: "Rango mínimo LRV",                type: "numero",   required: true },
    { key: "rango_max",          label: "Rango máximo URV",                type: "numero",   required: true },
    { key: "unidad_ing",         label: "Unidad de ingeniería",            type: "select",   required: true,  options: ["RPM", "Hz", "m/s", "rad/s", "in/s"] },
    { key: "senal_salida",       label: "Señal de salida",                 type: "select",   required: true,  options: ["4-20 mA HART", "4-20 mA", "Pulsos / frecuencia (HTL)", "Pulsos / frecuencia (TTL)", "0-10 V", "Interruptor (switch)"] },
    { key: "pulsos_por_rev",     label: "Pulsos por revolución (PPR)",     type: "numero",   required: false, validations: { unit: "PPR", min: 1 } },
    { key: "dientes_rueda",      label: "Nº dientes rueda fónica",         type: "numero",   required: false, validations: { min: 1 } },
    { key: "umbral_alarma_low",  label: "Setpoint ALARMA baja velocidad",  type: "numero",   required: false },
    { key: "umbral_alarma_high", label: "Setpoint ALARMA alta velocidad",  type: "numero",   required: false },
    { key: "umbral_overspeed",   label: "Setpoint PARO overspeed",         type: "numero",   required: true },
    { key: "umbral_underspeed",  label: "Setpoint PARO underspeed (opt.)", type: "numero",   required: false },
    { key: "cal_0_aplicado",     label: "Velocidad aplicada  0 % (LRV)",  type: "numero",   required: true },
    { key: "cal_0_leido",        label: "Señal leída  0 %",               type: "numero",   required: true },
    { key: "cal_25_aplicado",    label: "Velocidad aplicada 25 %",         type: "numero",   required: true },
    { key: "cal_25_leido",       label: "Señal leída 25 %",               type: "numero",   required: true },
    { key: "cal_50_aplicado",    label: "Velocidad aplicada 50 %",         type: "numero",   required: true },
    { key: "cal_50_leido",       label: "Señal leída 50 %",               type: "numero",   required: true },
    { key: "cal_75_aplicado",    label: "Velocidad aplicada 75 %",         type: "numero",   required: true },
    { key: "cal_75_leido",       label: "Señal leída 75 %",               type: "numero",   required: true },
    { key: "cal_100_aplicado",   label: "Velocidad aplicada 100 % (URV)", type: "numero",   required: true },
    { key: "cal_100_leido",      label: "Señal leída 100 %",              type: "numero",   required: true },
    { key: "func_overspeed_ok",  label: "Función PARO overspeed verificada", type: "checkbox", required: true, options: ["OK", "FALLA", "N/A"] },
    { key: "func_underspeed_ok", label: "Función PARO underspeed verificada", type: "checkbox", required: false, options: ["OK", "FALLA", "N/A"] },
    { key: "ajuste_cero",        label: "Ajuste de cero realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "ajuste_span",        label: "Ajuste de span realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "foto_evidencia",     label: "Foto tacómetro / evidencia",      type: "imagen",   required: false },
    { key: "obs_velocidad",      label: "Observaciones",                   type: "textarea", required: false },
    { key: "resultado_vel",      label: "Resultado",                       type: "select",   required: true,  options: ["APROBADO", "RECHAZADO"] },
  ],
};

const SECTION_CAUDAL_MASICO: MockInspectionSection = {
  id: "sec-caudal-masico",
  code: "CALIBRACION_CAUDAL_MASICO",
  name: "Calibración y Verificación de Caudal Másico",
  is_universal: false,
  fields: [
    { key: "tipo_caudalimetro",  label: "Tipo de caudalímetro másico",     type: "select",   required: true,  options: ["Coriolis (masa directa)", "Térmico — gases (dispersión térmica)", "Pesaje dinámico (belt weigher)", "Pérdida en peso (loss-in-weight)", "Coriolis + densímetro"] },
    { key: "fluido",             label: "Fluido / gas en proceso",         type: "texto",    required: true },
    { key: "rango_min_masico",   label: "Rango mínimo LRV (kg/h)",        type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "rango_max_masico",   label: "Rango máximo URV (kg/h)",        type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "unidad_ing",         label: "Unidad de ingeniería",            type: "select",   required: true,  options: ["kg/h", "t/h", "kg/min", "kg/s", "lb/h", "g/s"] },
    { key: "densidad_fluido",    label: "Densidad del fluido (kg/m³)",     type: "numero",   required: false, validations: { unit: "kg/m³" } },
    { key: "temperatura_proc",   label: "Temperatura proceso (°C)",        type: "numero",   required: false, validations: { unit: "°C" } },
    { key: "senal_caudal",       label: "Señal salida caudal",             type: "select",   required: true,  options: ["4-20 mA HART", "4-20 mA + Pulso", "Fieldbus", "PROFIBUS PA"] },
    { key: "senal_densidad",     label: "Señal salida densidad (si aplica)", type: "select", required: false, options: ["4-20 mA HART", "4-20 mA", "No aplica"] },
    { key: "factor_k",           label: "Factor K / factor de calibración",type: "numero",   required: false },
    { key: "tubo_lleno",         label: "Verificación cero (tubo lleno, flujo nulo)", type: "checkbox", required: true, options: ["OK", "FALLA", "N/A"] },
    { key: "cero_masico_leido",  label: "Lectura cero másico (kg/h)",      type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "cal_0_aplicado",     label: "Caudal aplicado  0 % (LRV)",     type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "cal_0_leido_ma",     label: "Señal leída  0 % (mA)",          type: "numero",   required: true,  validations: { unit: "mA" } },
    { key: "cal_25_aplicado",    label: "Caudal aplicado 25 %",            type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "cal_25_leido_ma",    label: "Señal leída 25 % (mA)",          type: "numero",   required: true,  validations: { unit: "mA" } },
    { key: "cal_50_aplicado",    label: "Caudal aplicado 50 %",            type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "cal_50_leido_ma",    label: "Señal leída 50 % (mA)",          type: "numero",   required: true,  validations: { unit: "mA" } },
    { key: "cal_75_aplicado",    label: "Caudal aplicado 75 %",            type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "cal_75_leido_ma",    label: "Señal leída 75 % (mA)",          type: "numero",   required: true,  validations: { unit: "mA" } },
    { key: "cal_100_aplicado",   label: "Caudal aplicado 100 % (URV)",    type: "numero",   required: true,  validations: { unit: "kg/h" } },
    { key: "cal_100_leido_ma",   label: "Señal leída 100 % (mA)",         type: "numero",   required: true,  validations: { unit: "mA" } },
    { key: "densidad_leida",     label: "Densidad leída (kg/m³)",          type: "numero",   required: false, validations: { unit: "kg/m³" } },
    { key: "error_densidad_pct", label: "Error densidad (%)",              type: "numero",   required: false, validations: { unit: "%" } },
    { key: "totalizador_ok",     label: "Totalizador de masa verificado",  type: "checkbox", required: false, options: ["OK", "FALLA", "N/A"] },
    { key: "ajuste_cero",        label: "Ajuste de cero realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "ajuste_span",        label: "Ajuste de span realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "foto_calibrador",    label: "Foto calibrador / evidencia",     type: "imagen",   required: false },
    { key: "obs_masico",         label: "Observaciones",                   type: "textarea", required: false },
    { key: "resultado_masico",   label: "Resultado",                       type: "select",   required: true,  options: ["APROBADO", "RECHAZADO"] },
  ],
};

const SECTION_VIBRACION: MockInspectionSection = {
  id: "sec-vibracion",
  code: "CALIBRACION_VIBRACION",
  name: "Calibración y Verificación de Vibración",
  is_universal: false,
  fields: [
    { key: "tipo_sensor",        label: "Tipo de sensor",                  type: "select",   required: true,  options: ["Acelerómetro piezoeléctrico", "Velocímetro (geófono)", "Sensor de proximidad (Eddy Current)", "Sensor sísmico", "MEMS acelerómetro"] },
    { key: "variable_medida",    label: "Variable medida",                 type: "select",   required: true,  options: ["Vibración (mm/s RMS)", "Aceleración (g)", "Desplazamiento (µm pico-pico)", "Velocidad (in/s RMS)"] },
    { key: "rango_min",          label: "Rango mínimo LRV",                type: "numero",   required: true },
    { key: "rango_max",          label: "Rango máximo URV",                type: "numero",   required: true },
    { key: "unidad_ing",         label: "Unidad de ingeniería",            type: "select",   required: true,  options: ["mm/s RMS", "g", "µm p-p", "in/s RMS", "m/s²"] },
    { key: "senal_salida",       label: "Señal de salida",                 type: "select",   required: true,  options: ["4-20 mA HART", "4-20 mA", "mV/g (acelerómetro)", "Modbus RTU", "Interruptor (switch)"] },
    { key: "tipo_montaje",       label: "Tipo de montaje",                 type: "select",   required: true,  options: ["Atornillado (perno M8)", "Magnético", "Adhesivo epóxico", "Brida / bridado"] },
    { key: "punto_montaje",      label: "Punto de montaje",                type: "select",   required: true,  options: ["Apoyo lado accionamiento (DE)", "Apoyo lado libre (NDE)", "Carcasa horizontal", "Carcasa vertical", "Rodamiento", "Otro"] },
    { key: "frecuencia_hz",      label: "Rango de frecuencia medido (Hz)", type: "texto",    required: false },
    { key: "umbral_alarma_h",    label: "Setpoint ALARMA HIGH",            type: "numero",   required: true },
    { key: "umbral_trip",        label: "Setpoint PARO / TRIP",            type: "numero",   required: true },
    { key: "valor_referencia",   label: "Valor vibrómetro referencia",     type: "numero",   required: true },
    { key: "valor_instrumento",  label: "Valor leído instrumento",         type: "numero",   required: true },
    { key: "error_absoluto",     label: "Error absoluto",                  type: "numero",   required: false },
    { key: "setpoint_alarma_ok", label: "Setpoint ALARMA HIGH verificado", type: "checkbox", required: true,  options: ["OK", "FALLA", "N/A"] },
    { key: "setpoint_trip_ok",   label: "Setpoint PARO/TRIP verificado",   type: "checkbox", required: true,  options: ["OK", "FALLA", "N/A"] },
    { key: "funcion_protec_ok",  label: "Función de protección verificada",type: "checkbox", required: true,  options: ["OK", "FALLA", "N/A"] },
    { key: "foto_evidencia",     label: "Foto vibrómetro / evidencia",     type: "imagen",   required: false },
    { key: "obs_vibracion",      label: "Observaciones",                   type: "textarea", required: false },
    { key: "resultado_vibracion",label: "Resultado",                       type: "select",   required: true,  options: ["APROBADO", "RECHAZADO"] },
  ],
};

const SECTION_POSICION: MockInspectionSection = {
  id: "sec-posicion",
  code: "CALIBRACION_POSICION",
  name: "Calibración y Verificación de Posición",
  is_universal: false,
  fields: [
    { key: "tipo_instrumento",   label: "Tipo de instrumento",             type: "select",   required: true,  options: ["Potenciómetro lineal", "LVDT (transductor diferencial)", "Encoder lineal incremental", "Encoder lineal absoluto", "Magnetostrictivo", "Posicionador de válvula (YT)", "Encoder rotativo", "Inclinómetro"] },
    { key: "rango_min",          label: "Rango mínimo LRV",                type: "numero",   required: true },
    { key: "rango_max",          label: "Rango máximo URV",                type: "numero",   required: true },
    { key: "unidad_ing",         label: "Unidad de ingeniería",            type: "select",   required: true,  options: ["mm", "cm", "m", "° (grados)", "% apertura", "pulsos", "in"] },
    { key: "senal_salida",       label: "Señal de salida",                 type: "select",   required: true,  options: ["4-20 mA HART", "4-20 mA", "0-10 V", "Encoder incremental (A/B)", "Encoder absoluto (SSI)", "Profibus PA", "Fieldbus"] },
    { key: "resolucion",         label: "Resolución declarada",            type: "texto",    required: false },
    { key: "tipo_acoplamiento",  label: "Tipo de acoplamiento",            type: "select",   required: false, options: ["Directo rígido", "Cable / hilo flexible", "Magnético sin contacto", "Cremallera / piñón", "Engranaje"] },
    { key: "cal_0_aplicado",     label: "Posición aplicada  0 % (LRV)",   type: "numero",   required: true },
    { key: "cal_0_leido",        label: "Señal leída  0 %",               type: "numero",   required: true },
    { key: "cal_25_aplicado",    label: "Posición aplicada 25 %",          type: "numero",   required: true },
    { key: "cal_25_leido",       label: "Señal leída 25 %",               type: "numero",   required: true },
    { key: "cal_50_aplicado",    label: "Posición aplicada 50 %",          type: "numero",   required: true },
    { key: "cal_50_leido",       label: "Señal leída 50 %",               type: "numero",   required: true },
    { key: "cal_75_aplicado",    label: "Posición aplicada 75 %",          type: "numero",   required: true },
    { key: "cal_75_leido",       label: "Señal leída 75 %",               type: "numero",   required: true },
    { key: "cal_100_aplicado",   label: "Posición aplicada 100 % (URV)",  type: "numero",   required: true },
    { key: "cal_100_leido",      label: "Señal leída 100 %",              type: "numero",   required: true },
    { key: "histeresis_medida",  label: "Histéresis medida",               type: "numero",   required: false },
    { key: "histeresis_max",     label: "Histéresis máxima admisible",     type: "numero",   required: false },
    { key: "ajuste_cero",        label: "Ajuste de cero realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "ajuste_span",        label: "Ajuste de span realizado",        type: "checkbox", required: true,  options: ["SI", "NO", "N/A"] },
    { key: "foto_calibrador",    label: "Foto calibrador / evidencia",     type: "imagen",   required: false },
    { key: "obs_posicion",       label: "Observaciones",                   type: "textarea", required: false },
    { key: "resultado_posicion", label: "Resultado",                       type: "select",   required: true,  options: ["APROBADO", "RECHAZADO"] },
  ],
};
...
export const MOCK_TEMPLATES: MockInspectionTemplate[] = [
  ...
  },
  {
    id: "tpl-ic-010",
    code: "P_IC_010",
    name: "Sensor / Transmisor de Vibración",
    discipline: "I&C",
    sections: [
      SECTION_DATOS_GENERALES,
      SECTION_INSPECCION_VISUAL,
  ...
  },
  {
    id: "tpl-ic-011",
    code: "P_IC_011",
    name: "Instrumento de Posición / Posicionador",
    discipline: "I&C",
    sections: [
      SECTION_DATOS_GENERALES,
      SECTION_INSPECCION_VISUAL,
  ...
  },
  {
    id: "tpl-ic-012",
    code: "P_IC_012",
    name: "Sensor / Transmisor de Velocidad",
    discipline: "I&C",
    sections: [
      SECTION_DATOS_GENERALES,
      SECTION_INSPECCION_VISUAL,
  ...
  },
  {
    id: "tpl-ic-013",
    code: "P_IC_013",
    name: "Caudalímetro de Caudal Másico",
    discipline: "I&C",
    sections: [
      SECTION_DATOS_GENERALES,
      SECTION_INSPECCION_VISUAL,
      SECTION_REDLINE,
      SECTION_CAUDAL_MASICO,
      SECTION_LOOP_CHECK,
      SECTION_FIRMAS,
    ],
  },
  {
    id: "tpl-ic-014",
    code: "P_IC_014",
    name: "Sensor / Transmisor de Torque",
    discipline: "I&C",
    sections: [
      SECTION_DATOS_GENERALES,
      SECTION_INSPECCION_VISUAL,
  ...
  },
  {
    id: "tpl-ic-015",
    code: "P_IC_015",
    name: "Celda de Carga / Transmisor de Peso y Fuerza",
    discipline: "I&C",
    sections: [
      SECTION_DATOS_GENERALES,
      SECTION_INSPECCION_VISUAL,
