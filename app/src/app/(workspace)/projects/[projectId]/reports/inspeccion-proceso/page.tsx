...
export default function InspeccionProcesoPage({ params }: Props) {
  const { projectId } = use(params);
  const { data, isLoading, isError } = useInspeccionReport(projectId, "proceso");
  ...
  async function handleExportXlsx() {
    ...
    const res = await fetch(
      `/api/reports/inspeccion-xlsx?projectId=${projectId}&discipline=proceso`,
    ...
    a.download = `INSPECCION_PROCESO_${name}_${today}.xlsx`;
    a.click();
  ...
  return (
    <>
      ...
      {data && <InspeccionPrintView data={data} title="LISTADO DE INSPECCIÓN DE EQUIPOS DE PROCESO" />}
    </>
  );
}
