...
export default function InspeccionICPage({ params }: Props) {
  const { projectId } = use(params);
  const { data, isLoading, isError } = useInspeccionReport(projectId, "ic");
  ...
  async function handleExportXlsx() {
    ...
    const res = await fetch(
      `/api/reports/inspeccion-xlsx?projectId=${projectId}&discipline=ic`,
    ...
    a.download = `INSPECCION_IC_${name}_${today}.xlsx`;
    a.click();
