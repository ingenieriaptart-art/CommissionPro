...
| 2 | Plantilla correcta (R3) | Confirmar que abre P_MEC_010 y que el flujo es por selector | a) Vía **URL directa**: confirmar header = "Motobomba" y que es P_MEC_010 (MB-TEST-001 NO está en el mapa, solo se abre por URL). b) **Mecanismo selector**: en el mapa de planta abrir una **bomba real existente** (que resuelve varias plantillas) y confirmar que aparece la lista **"Plantillas (N)"** y que el técnico **elige** (no auto-abre). NO asignar P_MEC_010 a equipos reales — solo observar el selector. | (a) Header = Motobomba; (b) el panel muestra lista de plantillas con elección manual (no apertura automática) | Captura header (URL directa) + captura del selector en una bomba | PASS si abre Motobomba por URL **y** el panel usa selector; FAIL si auto-abre sin elegir |
